...
- 数学公式渲染：集成 MathJax，支持完整的 LaTeX 语法
...
系统集成了 MathJax，支持完整的 LaTeX 数学公式语法：
...
### 支持的语法示例
- 复杂表达式：`$O(\log(\text{Range}) \cdot N^2)$`
- 分数和积分：`$\frac{a}{b}$, $\int_a^b f(x) \, dx$`
- 求和符号：`$\sum_{i=1}^n x_i$`
- 希腊字母：`$\alpha, \beta, \gamma$`
- 文本混排：`$\text{时间复杂度} O(n\log n)$`
